...
class Swiper {
	constructor(options = {}) {
		...
		this._scaleSize = this._direction === "horizontal" ? width : height;

		this.initSetIndex(options.index)

		this.initFromEffect(this._effect)
	...
	initSetIndex(index) {
		...
		index = Math.min(Math.abs(index), this._length - 1)
	...
	moveTo(index) {
		...
		if (this._loop) {
			if (index === min) {
				...
				this._defaultIndex = index;
				this._numericalConversion(this._defaultIndex);
				const s = Math.abs(oldIndex - Math.abs(this._defaultIndex)) * speed < 100 ? 100 : Math.abs(oldIndex - Math.abs(
					this._defaultIndex)) * speed;
